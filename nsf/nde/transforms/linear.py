...
class NaiveLinear(Linear):
    ...
    def inverse_no_cache(self, inputs):
        ...
        # outputs, lu = torch.solve(outputs.t(), self._weight)   # 之前的

        lu, pivots = torch.lu(self._weight)
        # P, L, U = torch.lu_unpack(lu, pivots)
        outputs = torch.lu_solve(outputs.t(), lu, pivots)
        ...
        logabsdet = -torch.sum(torch.log(torch.abs(torch.diag(lu))))    # torch.diag(lu)
        logabsdet = logabsdet * torch.ones(batch_size, device=inputs.device)
        return outputs, logabsdet
